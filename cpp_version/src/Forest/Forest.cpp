...
Forest::Forest() :
    ...
    NAN), importance_mode(DEFAULT_IMPORTANCE_MODE), regularization_usedepth(false), cr_impute_subdist(false), cr_impute_subdist_only_in_root(false), progress(0) {
}
...
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint mtry,
    ...
    const std::vector<double>& regularization_factor, bool regularization_usedepth, bool cr_impute_subdist,
  ...
  init(loadDataFromFile(input_file), mtry, output_prefix, num_trees, seed, num_threads, importance_mode,
      ...
      false, max_depth, regularization_factor, regularization_usedepth, cr_impute_subdist, cr_impute_subdist_only_in_root);
...
void Forest::initR(std::unique_ptr<Data> input_data, uint mtry, uint num_trees, std::ostream* verbose_out, uint seed,
    ...
    bool regularization_usedepth, bool cr_impute_subdist, bool cr_impute_subdist_only_in_root) {
  ...
  init(std::move(input_data), mtry, "", num_trees, seed, num_threads, importance_mode, min_node_size,
      ...
      regularization_factor, regularization_usedepth, cr_impute_subdist, cr_impute_subdist_only_in_root);
...
void Forest::init(std::unique_ptr<Data> input_data, uint mtry, std::string output_prefix,
    ...
    uint max_depth, const std::vector<double>& regularization_factor, bool regularization_usedepth, bool cr_impute_subdist,
    bool cr_impute_subdist_only_in_root) {
...
void Forest::grow() {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    trees[i]->init(data.get(), mtry, num_samples, tree_seed, &deterministic_varIDs, tree_split_select_weights,
        ...
        &regularization_factor, regularization_usedepth, &split_varIDs_used, cr_impute_subdist, cr_impute_subdist_only_in_root);
  }
